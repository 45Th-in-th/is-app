...
def pdf_to_text(pdf_path):
    ...
    images = convert_from_path(pdf_path, poppler_path=r'C:\poppler\bin')  # เพิ่ม poppler_path
    ...
    for i, image in enumerate(images):
        ...
        text = pytesseract.image_to_string(image, lang='tha')  # ถ้าเป็นภาษาไทยให้ใช้ lang='tha'
        all_text += text
...
def main():
    ...
    if file_type.endswith(".pdf"):
        ...
        with open(file_type, "w", encoding='utf-8') as txt_file:
            txt_file.write(text_from_pdf)
        print(f"ข้อความถูกบันทึกลงใน {file_type} เรียบร้อยแล้ว (TXT)")
    else:
